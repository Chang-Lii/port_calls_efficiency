...
This study investigates why median port times differ between countries and ship type, using aggregated vessel-arrival data from UNCTAD’s Port Call and Performance Statistics. Since port-time durations are highly right-skewed, the median port time was log-transformed. A sequence of linear regression models was then used to assess how vessel characteristics, ship-type categories, country indicators, and their interaction terms relate to log median port time.

Comparing models with AIC, adjusted R², and nested F-tests shows that vessel size, ship type, country fixed effects, and the interaction between size and ship type best explain group differences. Larger vessels tend to have longer median port times, whereas vessel age does not show a clear association. Ship type and country effects remain important after accounting for vessel characteristics.

Overall, the results indicate that both vessel structure and broader operational conditions influence differences in port-time performance across country and ship-type groups.These results help clarify which structural and operational factors most consistently explain differences in port efficiency across regions.

# Introduction {#sec-intro}

Maritime transport carries more than 80% of global trade. One important measure of port performance is time in port, which is the time vessels spend loading, unloading, and performing other operations. Even small changes in port time can add up across many ship visits, affecting shipping costs, supply chain reliability, and environmental impact. Knowing what causes differences in port time is important to policymakers, port authorities, and shipping firms.

Most research focuses on vessel-level or port-level factors, such as congestion or infrastructure. However, few studies examine how vessel characteristics and ship type together explain differences by country and ship type level. This paper addresses that gap by examining long-term factors like fleet makeup, regulations, and ship type. It investigates whether group-level vessel traits, especially average vessel age and size, along with ship type, are connected to differences in median port time across countries and ship types.

The study uses aggregated data from UNCTAD’s Port Call and Performance Statistics to build several linear regression models, each adding more detail such as ship type, country, and their interactions. Comparing these models shows that vessel size, ship type, country, and the interaction between size and ship type explain much of the variation in median port time. Vessel age, however, has little effect. These results point to broader patterns in maritime operations rather than differences between individual vessels. The paper is organised as follows: Section 2 describes the data; Section 3 outlines the modelling approach; Section 4 presents the results and diagnostics; and Section 5 discusses implications and limitations.

# Data {#sec-data}

## Introduction of Variables

The dataset used in this study is the 2023 release of UNCTAD Port Call and Performance Statistics (https://unctadstat.unctad.org/datacentre/dataviewer/US.PortCalls). This dataset integrates Automatic Identification System (AIS) vessel-tracking data with MarineTraffic port-mapping intelligence. The data are aggregated by country and ship type, and include vessels of 1,000 gross tonnage (GT) or greater. For each group, UNCTAD provides summary measures of vessel fleet characteristics, such as average age and gross tonnage, in addition to port-time performance indicators [@UNCTAD_PortCall].
...
Each observation in the dataset is a unique combination of country and ship type. The main variables in the analysis are the response variable, **median time in port**, and several group-level vessel characteristics: average vessel age, average gross tonnage (Size), commercial market segment (ShipType), and economy identifier (Country). These variables together describe the economic context, commercial category, and structural features of vessel groups, which are the foundation of the modelling framework.

Diagnostic checks reveal that the raw median port-time variable is highly right-skewed with a long upper tail. Using a logarithmic transformation makes the distribution more symmetric and less affected by extreme values, which helps prepare the data for linear regression. Therefore, all later models use $\log(\text{median time in port})$ as the response variable.
...
Different ship types are subject to distinct loading, unloading, and port-handling constraints. To address these systematic operational differences across ship types, indicator variables for ship-type categories are incorporated:
$$
\log(\text{Median Time In Port}_i)
= \beta_0
+ \beta_1\,\text{Age}_i
...
All hypothesis tests were done only on the final model chosen through model comparison. The t-tests and F-tests together show which vessel features, ship-type categories, and country-level factors are associated with differences in log median port time across countries and ship types.

All statistical modeling and diagnostics were conducted in R language[@R_language].
...
### Estimate Coefficent
**The full coefficient table is shown in Appendix (Figure 4).**

The estimated coefficients for Model Int-1 come from the final regression model(Model Int-1). Since the data are grouped by country and ship type, each coefficient shows the differences between these groups, not between individual vessels or ports. The response variable is the log of median port time, so all effects represent percentage changes in median port time.
...
Some ship types show statistically significant differences compared to the reference category. For example, Dry Bulk carriers $(p = 0.034)$ and LNG carriers $(p = 0.037)$ have higher log median port times after adjusting for vessel size, country, and the size-by-ship-type interaction. Holding all other terms constant, these coefficients represent group-level shifts in log port time, suggesting that features specific to these ship types may contribute to longer port times.

The interaction terms between vessel size and ship type are mostly not statistically significant at the 5% level, except for LNG carriers, which is marginally significant $(p = 0.056)$. This provides limited evidence that the relationship between vessel size and median port time differs across ship types. The marginal result for LNG carriers may indicate a slightly different size effect for this group, but the evidence is not strong enough to be conclusive.
...
In summary, the reasonable residual error and moderate adjusted $R^2$ show that the final model is suitable for estimating how vessel characteristics and ship types relate to median port times at the country and ship-type level.

### ANOVA

#### ANOVA Table for Model Int1 {#tab-anova}
![ANOVA](Tables/ANOVA.png){fig-align="center"}
...
Country indicators also make a statistically significant contribution $(F = 3.62,\; p < 9.7\times10^{-6})$. This means that even after considering vessel characteristics and ship type, there are still important differences between countries, likely due to institutional, regulatory, or infrastructure factors.
...
Overall, these ANOVA results show that vessel size, ship-type, country-level characteristics, and the interaction between size and ship type all play important roles in explaining variation in log median port time at the aggregated country–ship-type level.
...
Despite these limits, the findings provide useful insight into consistent patterns in maritime operations. They show that port-time differences between countries and ship types are not random but are linked to vessel size, ship type, and national operating conditions. Future research could build on this by using port-level or vessel-level data, studying nonlinear effects of vessel size, or adding outside measures of port efficiency. These steps would help explain factors that are not visible in the grouped data used here.

# Reproducibility

All analysis code, raw data, and documentation are available in GitHub repository:\
<https://github.com/Chang-Lii/port_calls_efficiency>

# Appendix {#sec-appendix}

![Estimated coefficients for Model Int-1](Tables/Model-int1_summary.png){fig-align="center"}
